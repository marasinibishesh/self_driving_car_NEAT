...
class NEATManager:
    ...
    def run_generation(self):
        ...
        gen_duration = datetime.now() - gen_start
        print(f"✅ Generation {self.generation} completed in {gen_duration.seconds} seconds")
        ...
        # Always save best model at the end of each generation
        if self.best_genome:
            self.save_best_model(self.best_genome)
        ...
        if (datetime.now() - self.last_generation_report).seconds >= 30:
            alive_count = len([g for g in self.population.population.values() if g.fitness > 0])
            print(f"⏳ Generation {self.generation}: Best fitness = {self.max_fitness:.2f}, " 
                  f"Species = {len(self.stats.get_species_sizes()[-1])}, "
                  f"Alive = {alive_count}/{self.config.pop_size}")
            self.last_generation_report = datetime.now()
    ...
    def eval_genomes(self, genomes, config):
        ...
        running = True
        last_save_check = datetime.now()  # Track last time we checked for saving
        
        while running and any(car.sprite.alive for car in cars):
            current_time = datetime.now()
            
            # Periodic save check during generation (FIXED)
            if current_time - self.last_save_time >= self.save_interval:
                if self.best_genome:
                    ...
                    print("⏱️ Periodic save: No best model available to save")
...
if __name__ == '__main__':
    ...
    print("- Models are automatically saved every 5 minutes and at the end of each generation")
